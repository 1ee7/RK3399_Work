...
static int gpio_demo_probe(struct platform_device *pdev)
{
  ...
  {
    pr_err("%s(): pinctrl not defined",__FUNCTION__);
  }
  else
  {
...
//=====CS
  ...
  //=====SDO 
   ...
   gpio_pinx[SPI_SDO] = of_get_named_gpio_flags(firefly_gpio_node,"spi-rx-gpio",0,&flag);
  //gpio_led = gpio_pinx[GPIO_3_D0];
  if(!gpio_is_valid(gpio_pinx[SPI_SDO]))
  {
     printk("spi-rx-gpio: %d is invalid\n", gpio_pinx[SPI_SDO]);
         return -ENODEV;
  }

  while(gpio_request(gpio_pinx[SPI_SDO],"spi-rx-gpio"))
  {
      printk("spi-rx-gpio %d request failed!\n", gpio_pinx[SPI_SDO]);
          gpio_free(gpio_pinx[SPI_SDO]);
          return -ENODEV;
  }

  gpio_direction_output(gpio_pinx[SPI_SDO],GPIO_LOW);
 ...
 //=====RST
  ...
  gpio_direction_output(gpio_pinx[SPI_RST],GPIO_LOW);
